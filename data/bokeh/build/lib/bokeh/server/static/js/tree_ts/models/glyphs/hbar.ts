  ...
  HBarView.prototype._render = function(ctx, indices, arg) {
    ...
    for (j = 0, len = indices.length; j < len; j++) {
      ...
      if (this.visuals.fill.doit) {
        ...
        x_val = this.renderer.model.data_source.attributes.data.right[i];
        y_val = this.renderer.model.data_source.attributes.data.y[i];
        this.data.bars.push({
          bbox: bbox,
          x: x_val,
          y: y_val,
          height: this.model.attributes.height.value
        });
      }
