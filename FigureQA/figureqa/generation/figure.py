...
class HBarGraphCategorical(object):
    def __init__(
        ...
        # Set up the plot
        p = figure(
            plot_width=visuals["figure_width"],
...
class Pie(object):
    def __init__(
        ...
        diff = 0
        if (
            "draw_legend" in visuals
            and visuals["draw_legend"]
            and visuals["legend_layout_position"] == "below"
        ):
        ...
        for i in range(len(data["colors"])):
            glyphs.append(
                p.wedge(
                    ...
                    y=0,
                    radius=1,
                    start_angle=data["starts"][i],
                    end_angle=data["ends"][i],
                    color=data["colors"][i],
                    name=data["labels"][i],
                )
            )
        ...
        if "draw_legend" in visuals and visuals["draw_legend"]:
            ...
            legend = Legend(
                items=[(data["labels"][i], [glyph]) for i, glyph in enumerate(glyphs)],
                name="the_legend",
                margin=0,
