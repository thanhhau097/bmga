...
from data_utils import (
    combine_source_and_rendered_data,
    get_best_inside_legend_position,
    hex_to_rgb,
)
from questions.categorical import (
    generate_bar_graph_questions,
    generate_pie_chart_questions,
)
...
def generate_data_by_shape(x_range, y_range, n, x_distn, shape):
    ...
    if shape == "random":
        ...
        y = np.clip(
            slope_direction * max_slope * np.random.random() * np.array(x[:]) + offset,
            y_range[0],
            y_range[1],
        ).tolist()
        ...
        y = np.clip(
            max_slope * np.random.random() * np.array(x[:]) + y_range[0],
            y_range[0],
            y_range[1],
        ).tolist()
        ...
        y = np.clip(
            -max_slope * np.random.random() * np.array(x[:]) + y_range[1],
            y_range[0],
            y_range[1],
        ).tolist()
        ...
        y = np.clip(
            np.array([a * (xx - h) ** 2 + k for xx in x]), y_range[0], y_range[1]
        ).tolist()
...
def _generate_scatter_data_continuous(
    x_range,
    y_range,
    x_distns,
    shapes,
...
def _generate_scatter_data_categorical(
    y_range, n_points_range, x_distns, shapes, n_classes_range, fix_y_range=False
):
    if not fix_y_range:
        y_range = pick_random_int_range(y_range)
    ...
    for i in range(0, n_classes):
        x_distn = np.random.choice(x_distns)
        shape = np.random.choice(shapes)
        x, y = generate_data_by_shape(
            [0, n_points - 1], y_range, n_points, x_distn, shape
        )
...
def generate_scatter():
    ...
    data = _generate_scatter_data_continuous(
        config["x_range"],
        config["y_range"],
        config["x_distn"],
        config["shape"],
...
def _generate_visuals_common():
    visuals = {}
    visuals["draw_legend"] = (
        True if np.random.random() <= common_config["draw_legend_pr"] else False
    )
    if visuals["draw_legend"]:
        visuals["legend_border"] = (
            True if np.random.random() <= common_config["legend_border_pr"] else False
        )
    ...
    visuals["draw_gridlines"] = (
        True if np.random.random() <= common_config["draw_gridlines_pr"] else False
    )

    visuals["legend_label_font_size"] = np.random.choice(
        common_config["legend_label_font_sizes"]
    )

    return visuals


def _generate_bar_categorical(key, data=None):
    config = data_config[key]

    if data is None:
        data = _generate_scatter_data_categorical(
            config["y_range"],
            config["n_points_range"],
            config["x_distn"],
            config["shape"],
    ...
    data["type"] = "scatter_categorical_base"
    data["data"] = [
        {
            "class": len(data["models"][0]["x"]) - 1,
            "x": data["models"][0]["y"],
            "y": data["models"][0]["x"],
        }
    ]
    data["n_points"] = len(data["models"][0]["x"])
    data["title"] = data["general_figure_info"]["title"]["text"]
    data["x_label"] = (
        data["general_figure_info"]["x_axis"]["label"]["text"]
        + " "
        + data["models"][0]["name"]
    )
    data["y_label"] = data["general_figure_info"]["y_axis"]["label"]["text"]
    data = {
        k: v
        for k, v in data.items()
        if k in ["type", "data", "n_points", "title", "x_label", "y_label"]
    }
...
def generate_hbar_categorical(real_data=None):
    bar_data = _generate_bar_categorical("hbar_categorical", real_data)
...
def _generate_visuals_for_line_plot(point_sets):
    ...
    visuals["legend_inside"] = (
        True if np.random.random() <= common_config["legend_inside_pr"] else False
    )
    ...
    if visuals["legend_inside"]:
        ...
        if (
            len(point_sets) <= common_config["legend_horizontal_max_classes"]
            and np.random.random() <= common_config["legend_horizontal_pr"]
        ):
        ...
        if (
            ...
            if visuals["figure_width"] < min_ratio * visuals["figure_height"]:
                visuals["figure_width"] = int(
                    (min_ratio + (max_ratio - min_ratio) * (np.random.random()))
                    * visuals["figure_height"]
                )

        legend_layout_position, legend_position = random.sample(
            outside_possibilities, 1
        )[0]
...
def generate_line():
    ...
    solid_only = (
        True if np.random.random() <= data_config["line"]["solid_pr"] else False
    )
    if solid_only:
...
def generate_pie():
    ...
    if visuals["draw_legend"]:
        ...
        if (
            n_classes <= common_config["legend_horizontal_max_classes"]
            and np.random.random() <= common_config["legend_horizontal_pr"]
        ):
            visuals["legend_orientation"] = "horizontal"
            outside_possibilities = [
...
def generate_source_data(
    ...
    PLOT_KEY_PAIRS = [
        ("vbar", "vbar_categorical"),
        ("hbar", "hbar_categorical"),
        ("pie", None),
        ("line", None),
        ("dot_line", None),
    ]
    ...
    local_vars = locals()
    if all(
        [local_vars[arg_name] == 0 for arg_name, actual_name in PLOT_KEY_PAIRS]
    ) or any([local_vars[arg_name] < 0 for arg_name, actual_name in PLOT_KEY_PAIRS]):
        raise Exception(
            "Invalid number of figures! Need at least one plot type specified!"
        )
    ...
    with open(real_data_json, "r") as f:
    ...
    for args_key, config_key in PLOT_KEY_PAIRS:
        ...
        real_data = [d for d in real_data if d["type"] == config_key]
        # real_data = real_data[: locals()[args_key]]

        # unpack to single series pair
        series_data = []
        for d in real_data:
                ...
                series_data.append(
                    {
                        "models": [model],
                        "type": d["type"],
                        "general_figure_info": d["general_figure_info"],
        ...
        generated_data = Parallel(n_jobs=os.cpu_count())(
            ...
            for i in tqdm(
                ...
                desc="Generating data for {:10}".format(args_key),
            )
        )
...
@click.option("-r", "--real-data-json", help="JSON file with real data")
@click.option(
    "-c",
    "--common-config-yaml",
    default=os.path.join("config", "common_source_data.yaml"),
    help="YAML file with common plotting and style attributes",
)
@click.option("--seed", default=1, type=int, help="seed for PRNGs")
@click.option(
    "--colors",
    default=os.path.join("resources", "x11_colors_refined.txt"),
...
@click.option("--vbar", default=0, type=int, help="number of vertical bar graphs")
@click.option("--hbar", default=0, type=int, help="number of horizontal bar graphs")
@click.option("--pie", default=0, type=int, help="number of pie charts")
@click.option("--line", default=0, type=int, help="number of line plots")
@click.option("--dot-line", default=0, type=int, help="number of dotted line plots")
def main(**kwargs):
    """
