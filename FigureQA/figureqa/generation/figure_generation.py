...
def generate_figures(
    source_data_json, destination_directory, add_bboxes=False, supplied_webdriver=None
):
    # Setup dest dirs
    ...
    def generate_figure(fig_id, source):
        # Create web driver
        ...
        try:
            ...
            N = 10
            title = source["title"]
            x_axis_label = source["x_label"]
            y_axis_label = source["y_label"]

            if fig_type == "vbar_categorical":
                fig = VBarGraphCategorical(
                    point_sets[0],
                    source["visuals"],
                    title=title,
            ...
            rendered_data = export_png_and_data(
                fig.figure, png_file, html_file, webdriver
            )
            ...
            annotations_json_file = os.path.join(
                annotations_json_dir, "%d_%s_annotations.json" % (fig_id, fig_type)
            )
                ...
                json.dump(
                    {
                        ...
                        "total_distinct_questions": source_data_json[
                            "total_distinct_questions"
                        ],
                        "total_distinct_colors": source_data_json[
            ...
            if add_bboxes:
                ...
                generate_all_images_with_bboxes_for_plot(
                    all_plot_data, png_file, bbox_img_dir, "red", load_image=True
                )
    ...
    Parallel(n_jobs=os.cpu_count())(
        delayed(generate_figure)(fig_id, source)
        for fig_id, source in tqdm(
            iter(enumerate(source_data_json["data"])),
