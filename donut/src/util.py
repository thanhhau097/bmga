...
class DataCollator:
    def __init__(self, use_text_inputs: bool = False):
        self.use_text_inputs = use_text_inputs

    def __call__(self, batch: List[Tuple[torch.Tensor]]):
        image_tensors, decoder_input_ids, decoder_labels = [], [], []
        prompt_end_idxs = []
        if self.use_text_inputs:
            encoder_input_ids = []
        ...
        for batch_data in batch:
            ...
            prompt_end_idxs.append(batch_data[3])
            if self.use_text_inputs:
                encoder_input_ids.append(batch_data[4])

        image_tensors = torch.stack(image_tensors)
        decoder_input_ids = torch.stack(decoder_input_ids)
        decoder_labels = torch.stack(decoder_labels)
        prompt_end_idxs = torch.stack(prompt_end_idxs)

        inputs = {
            "image_tensors": image_tensors,
            "decoder_input_ids": decoder_input_ids,
            "decoder_labels": decoder_labels,
            "prompt_end_idxs": prompt_end_idxs,
        }
        if self.use_text_inputs:
            encoder_input_ids = torch.stack(encoder_input_ids)
...
class DonutDataset(Dataset):
    ...
    def __init__(
        ...
        self.use_text_inputs = True if hasattr(donut_model, "text_encoder_tokenizer") else False

        try:
            self.dataset = load_dataset(dataset_name_or_path, split=self.split).cast_column(
                "image", Image(decode=False)
            )
        except ValueError:
            dataset = []
            for json_str in list(
                open(os.path.join(dataset_name_or_path, self.split, "metadata.jsonl"))
            ):
                result = json.loads(json_str)
                result["image"] = os.path.join(
                    dataset_name_or_path, self.split, result["file_name"]
                )
        ...
        for sample in tqdm(self.dataset):
            ...
            if self.use_text_inputs:
                assert "text" in ground_truth, "text field is required for text inputs"
                self.text_sequences.append(" ".join(ground_truth["text"]))
        ...
        self.prompt_end_token_id = self.donut_model.decoder.tokenizer.convert_tokens_to_ids(
            self.prompt_end_token
        )
        self.max_char_length = max(
            [len(gt_token_sequence[0]) for gt_token_sequence in self.gt_token_sequences]
        )
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.use_text_inputs:
            encoder_input_ids = self.donut_model.text_encoder_tokenizer(
            ...
            return input_tensor, input_ids, labels, prompt_end_index, encoder_input_ids
        return input_tensor, input_ids, labels, prompt_end_index
