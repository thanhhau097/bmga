...
class ImageDataset(data.Dataset, Configurable):
    ...
    def get_all_samples(self):
        for i in range(len(self.data_dir)):
            ...
            if self.is_training:
                image_path=[self.data_dir[i]+'/train/images/'+timg.strip() for timg in image_list]
                gt_path=[self.data_dir[i]+'/train_gts/'+timg.strip()+'.txt' for timg in image_list]
            else:
                image_path=[self.data_dir[i]+'/validation/images/'+timg.strip() for timg in image_list]
                print(self.data_dir[i])
                if 'TD500' in self.data_list[i] or 'total_text' in self.data_list[i]:
                    gt_path=[self.data_dir[i]+'/val_gts/'+timg.strip()+'.txt' for timg in image_list]
                else:
                    gt_path=[self.data_dir[i]+'/val_gts/'+'gt_'+timg.strip().split('.')[0]+'.txt' for timg in image_list]
